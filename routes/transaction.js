...
router.post('/save', auth, async (req, res) => {
  ...
  if (!paidByUser) return res.status(403).send('Paid By user selected is not a valid User');
  ...
  if (!paidForUsers) return res.status(403).send('One or manyPaid For user are not valid');

  let transaction = new Transaction({
    date: req.body.date,
    paidBy: _.pick(paidByUser, ['_id', 'name', 'phone']),
    paidFor: _.map(paidForUsers, user => { return { _id: user._id, name: user.name, phone: user.phone }; }),
...
function validate(obj) {
  const schema = {
    ...
    transactionType: Joi.string().valid('Myself', 'Group', 'owe').required(),
...
function validatePaidByMeToUsers(obj) {
  const schema = {
    toUserIds: Joi.array().items(Joi.objectId()).required()
  };
  return Joi.validate(obj, schema);
}
